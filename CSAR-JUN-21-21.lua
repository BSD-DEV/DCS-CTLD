...
csar.aircraftType["Mi-24P"] = 3

--Fargo 9/20
csar.usemashzones = false
csar.mashzones = {"haifahospital",}
...
--tupper beg
        ...
        elseif world.event.S_EVENT_LAND == _event.id then
            ...
            if csar.allowFARPRescue then
                ...
                --Fargo 
                -- csar.mashzones is a table of UNIT NAMES that are late act. polygonal zones                
                _heliPoint = _unit:getPoint()
                if csar.usemashzones == true then
                  for _i, _polyzone in pairs(csar.mashzones) do
                    env.info("DEBUG: INSIDE MASHZONESLOOP")
                    IsinZone = mist.pointInPolygon(_heliPoint, mist.getGroupPoints(_polyzone))
                    if IsinZone == true then
                    env.info("DEBUG: INSIDE MASHZONESLOOP, RESCUEPILOTS!")                   
                ...
                if _place == nil and inZone == false then
...
function csar.spawnGroup( _coalition, _country, _point, _typeName )
    ...
    if _typeName == "SA342Mistral" or _typeName == "SA342Minigun" or _typeName == "SA342L" or _typeName == "SA342M"
     or _typeName == "UH-1H" or _typeName == "Mi-8MT" or _typeName == "Mi-24P"  then
       _double = true
    end
