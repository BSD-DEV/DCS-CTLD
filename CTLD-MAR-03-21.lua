...
--assert(loadfile("C:\\Users\\Fargo\\Downloads\\Scripts\\scriptname.lua"))()
...
ctld.wpZones = {
	...
	{ "azone1", "none","yes", 0 ,"Off Road"},
	{ "azone2", "none","yes", 0 ,"Off Road"},
	{ "azone3", "none","yes", 0 ,"Off Road"},
	{ "azone4", "none","yes", 0 ,"Off Road"},
	{ "azone5", "none","yes", 0 ,"Off Road"},
	{ "azone6", "none","yes", 0 ,"Off Road"},
	{ "azone7", "none","yes", 0 ,"Off Road"},
	{ "szone1", "none","yes", 0 ,"Off Road"},
	{ "szone2", "none","yes", 0 ,"Off Road"},
	{ "szone3", "none","yes", 0 ,"Off Road"},
	{ "szone4", "none","yes", 0 ,"Off Road"},
	{ "szone5", "none","yes", 0 ,"Off Road"},
	{ "szone6", "none","yes", 0 ,"Off Road"},
	{ "szone7", "none","yes", 0 ,"Off Road"},
	{ "szone8", "none","yes", 0 ,"Off Road"},
	{ "szone9", "none","yes", 0 ,"Off Road"},
	{ "szone10", "none","yes", 0 ,"Off Road"},
}
...
az.actionZones = {
	...
	{ "azone1", "100" ,"101" , true, nil },
    { "azone2", "200" , "202", false, "azone3" },  -- Won't return to dropped point but proceed to azone3. To do multiple task
    { "azone3", "300" , "303", false, "azone4" },  -- Won't return to dropped point but proceed to azone4. To do multiple task
	{ "azone4", "400" , "404", false, "azone5" },
	{ "azone5", "500" ,"505" , true, nil },
    { "azone6", "600" , "606", false, "wpzone3" },  -- Won't return to dropped point but proceed to wp3. To do multiple task
    { "azone7", "700" , nil, false, nil },  -- To use as an extraction point
	{ "szone1", "811" , "821", true, nil },
	{ "szone2", "812" , "822", true, nil },
	{ "szone3", "813" , "823", true, nil },
	{ "szone4", "814" , "824", true, nil },
	{ "szone5", "815" , "825", false, "szone6" },
	{ "szone6", "816" , "826", false, "szone7" },
	{ "szone7", "817" , "827", false, "szone8" },
	{ "szone8", "818" , "828", false, "szone9" },
	{ "szone9", "819" , "829", false, "szone10" },
	{ "szone10", "820" , "830", false, nil },
    }
az.zoneFlgDistance = 40 -- Meters from the center of the zone to trigger the in zone flag     
az.groupsdropped = {}    
...
-- Use any of the predefined names or set your own ones
ctld.transportPilotNames = {"transport1","transport2","transport3","iedvehicle"}
...
ctld.loadableGroupsbyType['UH-1H'] = {
                        {name = "Standard Group", inf = 6, mg = 2  }, -- will make a loadable group with 5 infantry, 2 MGs and 2 anti-tank for both coalitions
...
ctld.loadableGroupsbyType['Mi-8MT'] = {
                        {name = "Standard Group", inf = 18, mg = 6}, -- will make a loadable group with 5 infantry, 2 MGs and 2 anti-tank for both coalitions
                        {name = "Anti Air", inf = 14, aa = 10  },
                        {name = "Anti Tank", at = 4  },
...
ctld.spawnableCrates = {
    ...
    ["Ground Forces"] = {
        ...
        { weight = 2663, desc = "HMMWV - TOW", unit = "M1045 HMMWV TOW", side = 2 },
        { weight = 2363, desc = "HMMWV - MG", unit = "M1043 HMMWV Armament", side = 2 },

        { weight = 510, desc = "BTR-D", unit = "BTR_D", side = 1 },
        { weight = 515, desc = "BRDM-2", unit = "BRDM-2", side = 1 },

        { weight = 2364, desc = "HMMWV - JTAC", unit = "Hummer", side = 2, }, -- used as jtac and unarmed, not on the crate list if JTAC is disabled
        { weight = 525, desc = "SKP-11 - JTAC", unit = "SKP-11", side = 1, }, -- used as jtac and unarmed, not on the crate list if JTAC is disabled

        { weight = 1000, desc = "RADAR SYSTEM", unit = "Silkworm_SR" },

        { weight = 250, desc = "SPH 2S19 Msta", unit = "SAU Msta", side = 1, cratesRequired = 3 },
        { weight = 2880, desc = "M-109", unit = "M-109", side = 2, cratesRequired = 3 },

        { weight = 2110, desc = "Ural-375 Ammo Truck", unit = "Ural-375", side = 1, cratesRequired = 2 },
        { weight = 2111, desc = "M-818 Ammo Truck", unit = "M 818", side = 2, cratesRequired = 2 },

        { weight = 1800, desc = "FOB Crate - Small", unit = "FOB-SMALL" }, -- Builds a FOB! - requires 3 * ctld.cratesRequiredForFOB
    },
    ["Supplies"] = {
        { weight = 611, desc = "Net Cargo (6-900kg)", unit = "uh1h_cargo" },
        { weight = 485, desc = "Gasoline (4-500 kg)", unit = "barrels_cargo" },        
      ...
      ["AA Crates"] = {
        ...
        { weight = 4418, desc = "SA15 Tor", unit = "Tor 9A331", side = 2 },
...
function ctld.generateTroopTypes(_side, _countOrTemplate, _country)
    ...
    if type(_countOrTemplate) == "table" then
        ...
        if _countOrTemplate.at then
            _troops = ctld.insertIntoTroopsArray("BSD Marine TOW",_countOrTemplate.at,_troops)
        end
        ...
        if _countOrTemplate.jtac then
            _troops = ctld.insertIntoTroopsArray("BSD Marine TOW",_countOrTemplate.jtac,_troops)
        end
   -- Tupperware end
   --Fargo 8/17
        if _countOrTemplate.ANGLICO then
            _troops = ctld.insertIntoTroopsArray("BSD Marine TOW",_countOrTemplate.ANGLICO,_troops)
        end
    ...
    if type(_countOrTemplate) == "table" then
      ...
      --Fargo 8/17
      elseif _countOrTemplate.ANGLICO then
          if _countOrTemplate.ANGLICO > 0 then      
          _details = { units = _troops, groupId = _groupId, groupName = string.format("ANGLICO %i", _groupId), side = _side, country = _country }
          env.info("DEBUG fjw 2137")
          else
              _details = { units = _troops, groupId = _groupId, groupName = string.format("Dropped Group %i", _groupId), side = _side, country = _country }
            end
      else
...
function ctld.extracttroopsAction( _args )
   ...
   -- Loop each unit
      ...
      if  ctld.inTransitTroops[_heli:getName()] == nil then
        ...
        elseif string.match( _onboard.troops.groupName, "ANGLICO") then
          ctld.inTransitTroops[_heli:getName()].troops.groupName = string.format("ANGLICO %i", _groupId)   
        else
        ...
        if  ctld.inTransitTroops[_heli:getName()].troops == nil then
          ...
          elseif string.match( _onboard.troops.groupName, "ANGLICO") then       
            ctld.inTransitTroops[_heli:getName()].troops.groupName = string.format("ANGLICO %i", ctld.inTransitTroops[_heli:getName()].troops.groupId)
        ...
        --            return
            --    end
                ...
                elseif _crate.details.cratesRequired ~= nil and _crate.details.cratesRequired > 1 then
                    ...
                    env.info("DEBUG 3561: unpack() _crate.details.unit " .. _crate.details.unit)
                    
                    if string.match(_crate.details.unit, "cargo") then
                      ctld.displayMessageToGroup(_heli, "Supplies are not to be unpacked by the air crew.\nNow get outta here before we send a strongly worded letter to the HC. ", 20)
...
function ctld.spawnCrateGroup(_heli, _positions, _types)
    ...
    if string.match(_groupName, "Silkworm") then
      _groupName = "ANGLICO" .. _id
      env.info("fjw DEbug: 4549")
    end
...
--Fargo 4456 (move dropped crate vehicles to wpzone
     env.info("fargo debug  4592")

    if string.match(_groupName, "Silkworm_SR") then
      
    end
    
    if string.match(_groupName, "M-109") then
     
     env.info("fargo debug arty reg 4603")
...
function ctld.spawnDroppedGroup(_point, _details, _spawnBehind, _maxSearch)
    ...
    local _spawnedGroup = Group.getByName(mist.dynAdd(_group).name)
      ...
      --Fargo 08/17
      if string.match(_name1, "ANGLICO") then
         local con = _spawnedGroup:getController()
         --AI.Option.Ground.val.ROE
         env.info("fjw setting ROE weapon hold")
         con:setOption(AI.Option.Ground.id.ROE, AI.Option.Ground.val.ROE.WEAPON_HOLD)  -- Set to only return fire   
      end 
...
function ctld.checkForHelis(  )
   ...
   local _allHeliGroups = coalition.getGroups(1, Group.Category.HELICOPTER)
   ...
   local _allHeliGroups2 = coalition.getGroups(2, Group.Category.HELICOPTER)
   --timer.scheduleFunction(ctld.checkForHelis, nil, timer.getTime() + 10)
   if _allHeliGroups2 ~= nil then
       for _key, _group in pairs (_allHeliGroups2) do
        local _unit = _group:getUnit(1) -- Asume that there is only one unit in the flight for players
          if _unit then
            if _unit:isExist() ~= true then
                return
...
function ctld.spawnf10Group(_country, _side, _positions, _types)
    ...
    env.info("FARGO DEBUG: 6983")
    if string.match(_groupName, "M-109") then
     
       env.info("fargo debug arty 6986")
...
function ctld.checkforf10Mark()
  ...
  timer.scheduleFunction(ctld.checkforf10Mark,nil,timer.getTime() + 10)
    ...
    for key, _mark in pairs (f10marks) do
      ...
      if string.match(text, "COORD") then
        local pos = _mark.pos
        local _flgused = false
        
        if string.match(text, "LL") then
        
              local _lat, _lon = coord.LOtoLL(pos)

              _coordStr = mist.tostringLL(_lat, _lon, 1 ,true)
              
              _flgused = true
        end

        if string.match(text, "LLDMS") then
        
              local _lat, _lon = coord.LOtoLL(pos)

              _coordStr = mist.tostringLL(_lat, _lon, 3)
              
              _flgused = true
        end
        
        if string.match(text, "MGRS") then
        
              _coordStr = mist.tostringMGRS(coord.LLtoMGRS(coord.LOtoLL(pos)), 5)
              --env.info("DEBUG: MGRS:" .. _coordStr)
              
              _flgused = true
        end        
        if string.match(text, "ALL") then
        
              _coordStr1 = mist.tostringMGRS(coord.LLtoMGRS(coord.LOtoLL(pos)), 5)
              
              local _lat, _lon = coord.LOtoLL(pos)
              _coordStr2 =  mist.tostringLL(_lat, _lon, 3)
              _coordStr3  = mist.tostringLL(_lat, _lon, 1,true)
              _coordStr = _coordStr1 .. "\n" .. _coordStr2 .. "\n" .. _coordStr3 
              --env.info("DEBUG: MGRS:" .. _coordStr)
              
              _flgused = true
        end        
        if _flgused == true then
            ctld.mark_num = ctld.mark_num + 1 
            trigger.action.markToAll(ctld.mark_num, _coordStr, pos)
     ...
     --     radius = tonumber(tab[2])
        ...
        if string.match(text, "TANK") then
          _spawnedGroups = ctld.spawnf10Group(2, 2, { pos }, {"M-1 Abrams"})
